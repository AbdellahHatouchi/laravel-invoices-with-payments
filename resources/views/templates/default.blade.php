...
<body>
    ...
    @if ($invoice->payments && count($invoice->payments) !== 0)
        ...
        <table class="table table-items mt-3">
            <thead>
                <tr>
                    <th scope="col" class="border-0 pl-0">{{ __('invoices::invoice.referance') }}</th>
                    <th scope="col" class="text-center border-0">{{ __('invoices::invoice.user') }}</th>
                    <th scope="col" class="text-center border-0">{{ __('invoices::invoice.method') }}</th>
                    <th scope="col" class="text-right border-0">{{ __('invoices::invoice.amount') }}</th>
                    <th scope="col" class="text-right border-0 pr-0">{{ __('invoices::invoice.created_at') }}</th>
                </tr>
            </thead>
            <tbody>
                {{-- Items --}}
                ...
                @endforeach
                {{-- Summary --}}
                @if ($invoice->total_payments)
                    <tr>
                        <td colspan="{{ $invoice->table_columns - 2 }}" class="border-0"></td>
                        <td class="text-right pl-0">{{ __('invoices::invoice.total_payments') }}</td>
                        <td class="text-right pr-0">
                            {{ $invoice->formatCurrency($invoice->total_payments) }}
                        </td>
                    </tr>
                    <tr>
                        <td colspan="{{ $invoice->table_columns - 2 }}" class="border-0"></td>
                        <td class="text-right pl-0">{{ __('invoices::invoice.belonce') }}</td>
                        <td class="text-right pr-0 total-amount">
                            {{ $invoice->formatCurrency($invoice->total_amount - $invoice->total_payments) }}
                        </td>
                    </tr>
                @endif
            </tbody>
